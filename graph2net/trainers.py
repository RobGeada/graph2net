...
def generate(cell_matrices, data, **kwargs):
    ...
    if kwargs.get('verbose', False):
        print(cell_matrices)
        show_cell(cell_matrices, kwargs.get('name', "Validating Cell"))
    ...
    if len(cell_types) > 1 and scales[0]>2:
        if kwargs.get('verbose', False):
    ...
    try:
        for epoch in range(kwargs['epochs']):
            if epoch and not (epoch+1) % 32:
                torch.save(model.state_dict(),"model_ckpt_{}".format(epoch))
            t_start = time()
            train(model,
                  device,
                  train_loader,
            ...
            loss, correct, preds = test(model, device, test_loader, verbose=kwargs.get('verbose',False))
